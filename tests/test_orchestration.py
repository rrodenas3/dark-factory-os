...
def test_finance_ap_run_reaches_terminal_state(graph) -> None:  # type: ignore[no-untyped-def]
...
def test_finance_ap_run_executes_tools(graph) -> None:  # type: ignore[no-untyped-def]
    final = graph.invoke(_initial("run-fin-002", "finance", "ap-exception-resolution"))
    assert len(final["tool_trace"]) >= 1


def test_finance_ap_cites_policy(graph) -> None:  # type: ignore[no-untyped-def]
...
def test_retail_promo_run_completes(graph) -> None:  # type: ignore[no-untyped-def]
    final = graph.invoke(_initial("run-ret-001", "retail", "promo-rebalance"))
    assert final["status"] in ("completed", "approval_required", "failed")


def test_saas_incident_run_completes(graph) -> None:  # type: ignore[no-untyped-def]
    final = graph.invoke(_initial("run-saas-001", "saas", "incident-triage"))
    assert final["status"] in ("completed", "approval_required", "failed")


def test_approval_required_when_financial_tool_in_plan(graph) -> None:  # type: ignore[no-untyped-def]
...
def test_tool_trace_records_risk_tier(graph) -> None:  # type: ignore[no-untyped-def]
...
def test_spend_anomaly_skill_completes(graph) -> None:  # type: ignore[no-untyped-def]
