...
_PURCHASE_ORDERS: dict[str, dict[str, Any]] = {
    ...
    "PO-5105": {
        "id": "PO-5105",
        "vendor": "Tailspin Supplies",
        "approved_amount": 740.00,
        "received": True,
        "vendor_risk": "elevated",
    },  # noqa: E501
...
_POLICY_CLAUSES: list[dict[str, Any]] = [
    {
        ...
        "text": "Invoices matching vendor, amount, and period must be flagged as duplicates and not paid.",
        "score": 0.97,
    },  # noqa: E501
    {
        "policy_id": "POL-AP-12",
        ...
        "text": "Invoice amounts exceeding PO by more than 5% require finance-manager approval above $1,000.",
        "score": 0.95,
    },  # noqa: E501
    {
        "policy_id": "POL-AP-18",
        "clause_id": "2.1",
        "title": "Missing PO",
        "text": "Services invoices without a matching PO must be escalated to the approver chain.",
        "score": 0.93,
    },  # noqa: E501
    {
        "policy_id": "POL-AP-18",
        ...
        "text": "Tax rounding differences below $1.00 may be approved automatically under POL-AP-18 clause 3.0.",
        "score": 0.89,
    },  # noqa: E501
    {
        "policy_id": "POL-AP-18",
        "clause_id": "3.5",
        "title": "Credit Memo Threshold",
        "text": "Credit memos above $500 require finance-manager sign-off.",
        "score": 0.91,
    },  # noqa: E501
    {
        "policy_id": "POL-AP-20",
        "clause_id": "1.0",
        "title": "Vendor Risk",
        "text": "Elevated-risk vendors trigger mandatory review regardless of invoice amount.",
        "score": 0.94,
    },  # noqa: E501
    {
        "policy_id": "POL-AP-20",
        "clause_id": "2.0",
        "title": "Void Invoice",
        "text": "Invoice voids are irreversible; an ActionReadinessPack and director approval are required.",
        "score": 0.99,
    },  # noqa: E501
    {
        "policy_id": "POL-PRICE-04",
        "clause_id": "1.0",
        "title": "Price Band",
        "text": "Price band changes must be approved by commercial-manager and remain within 15% of baseline.",
        "score": 0.96,
    },  # noqa: E501
    {
        "policy_id": "POL-SPEND-01",
        "clause_id": "1.1",
        "title": "Spend Anomaly",
        "text": "Vendor category spend exceeding 2x rolling 90-day average triggers an anomaly review.",
        "score": 0.90,
    },  # noqa: E501
]
...
def _erp_get_purchase_order(args: dict[str, Any]) -> ToolResult:
    ...
    if po is None:
        return ToolResult(
            tool_name="erp.get_purchase_order",
            success=False,
            output=None,
...
def _policy_search(args: dict[str, Any]) -> ToolResult:
    ...
    matches = [
        c for c in _POLICY_CLAUSES if query in c["title"].lower() or any(w in c["text"].lower() for w in query.split())
    ]  # noqa: E501
    if not matches:
    ...
    return ToolResult(
        ...
        success=True,
        output={"matches": matches, "domain": domain},
        latency_ms=latency,
        cost_usd=0.001,
    )  # noqa: E501
...
def _memory_search(args: dict[str, Any]) -> ToolResult:
    ...
    if "incident" in namespace:
        summary = (
            "billing-api P1 incidents historically follow deploys touching invoice-preview or payment-retry paths."  # noqa: E501
        )
    latency = int((time.monotonic() - t0) * 1000) + random.randint(30, 90)
    return ToolResult(
        tool_name="memory.search",
        success=True,
        output={"namespace": namespace, "summary": summary, "trust": 0.87},
        latency_ms=latency,
        cost_usd=0.0,
    )  # noqa: E501
...
def _analytics_campaign(args: dict[str, Any]) -> ToolResult:
    ...
    return ToolResult(
        ...
        success=True,
        output={
            "campaign_id": args.get("campaign_id", "CMP-100"),
            "margin_delta": -0.043,
            "revenue": 48200.0,
            "units_sold": 1840,
            "stockout_minutes": 12,
...
def _analytics_incident(args: dict[str, Any]) -> ToolResult:
    ...
    return ToolResult(
        ...
        success=True,
        output={
            "service": args.get("service", "billing-api"),
            "p1_count": 3,
            "error_rate_pct": 4.7,
            "deploy_sha": "a3f9c1d",
        },  # noqa: E501
...
def _telemetry_deployments(args: dict[str, Any]) -> ToolResult:
    ...
    return ToolResult(
        ...
        output={
            "service": args.get("service", "billing-api"),
            "recent_deploys": [
                {"sha": "a3f9c1d", "deployed_at": "2026-06-17T22:14Z", "impact": "invoice-preview-refactor"}
            ],
        },  # noqa: E501
        latency_ms=latency,
        cost_usd=0.0,
